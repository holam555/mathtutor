...
export function RedemptionButtons({ id, status }: { id: string; status: string }) {
  ...
  if (done === 'approved') {
    return <span className="text-xs font-medium text-green-600 bg-green-50 px-2 py-1 rounded-full">已批准</span>
  }
  if (done === 'rejected') {
    return <span className="text-xs font-medium text-gray-400 bg-gray-100 px-2 py-1 rounded-full">已拒絕</span>
  }
