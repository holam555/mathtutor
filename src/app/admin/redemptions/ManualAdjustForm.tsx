...
export default function ManualAdjustForm({ students }: { students: Student[] }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white rounded-2xl p-5 shadow-sm space-y-3">
      <h3 className="font-semibold text-gray-800">{t('手動調整 Token 餘額')}</h3>

      <div>
        <label className="block text-xs font-medium text-gray-500 mb-1">{t('選擇學生')}</label>
        <select
          value={studentId}
        ...
        >
          <option value="">{t('選擇學生…')}</option>
          {students.map((s) => (
            <option key={s.id} value={s.id}>
              {lang === 'en'
                ? `${s.name} (currently ${s.token_balance ?? 0} credits)`
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          ...
          <input
            ...
            required
            placeholder={t('例如：50 或 -20')}
            className="w-full px-3 py-2 border border-gray-200 rounded-xl text-sm focus:border-[#4A90E2] outline-none"
          />
        </div>
        <div>
          <label className="block text-xs font-medium text-gray-500 mb-1">{t('備注（可選）')}</label>
          <input
            type="text"
            value={note}
            onChange={(e) => setNote(e.target.value)}
            placeholder={t('例如：特別活動獎勵')}
            className="w-full px-3 py-2 border border-gray-200 rounded-xl text-sm focus:border-[#4A90E2] outline-none"
          />
        </div>
      </div>
      ...
      {result?.success && <p className="text-xs text-green-600">✓ {t('已調整')}</p>}
