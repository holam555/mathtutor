...
//   Legacy mode (P5/P6): ?grade=5&month=9  → reads hardcoded data
//   New P3 mode: ?grade=3&unit_ids=uuid,uuid (or topic_ids=uuid,uuid)
export async function GET(request: NextRequest) {
  ...
  // ─── P3 new path: load curriculum + select from assessment_questions ────
  ...
  const result = selectQuestions(candidateRows, scopes)
