    ...
    return (
      <div className="space-y-4">
        <div className="bg-white rounded-2xl p-6 shadow-sm text-center">
          ...
          <h2 className="font-bold text-gray-800 text-lg mb-1">{t('模擬考試範圍已設定')}</h2>
          <p className="text-sm text-gray-500 mb-4">
            {selectedChild?.name ?? t('學生')} · {confirmedUnits.length} {t('個單元')}
          </p>
        ...
        <div className="bg-[#F7FBF9] border border-[#1D9E75]/20 rounded-2xl p-5">
          <p className="text-sm font-semibold text-[#1D9E75] mb-3">📝 {t('接下來會點？')}</p>
          ...
          <div className="bg-gradient-to-br from-[#EF9F27] to-[#F8B84E] rounded-xl p-3 shadow-sm mb-1.5">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-bold text-white">📝 {t('模擬考試試卷')}</p>
                <p className="text-[10px] text-white/85 mt-0.5">{t('40 題 · 50 分鐘')}</p>
              </div>
          ...
          <p className="text-[11px] text-gray-500 text-center mb-4">
            {lang === 'en'
              ? `↑ This card will appear on ${selectedChild?.name ?? 'the student'}'s home page`
          ...
          <div className="space-y-2.5">
            <div className="flex items-start gap-3">
              ...
              <p className="text-xs text-gray-700 leading-relaxed">
                {lang === 'en'
                  ? <>The student taps the card and downloads the <strong>long-answer PDF</strong></>
            ...
            <div className="flex items-start gap-3">
              ...
              <p className="text-xs text-gray-700 leading-relaxed">
                {lang === 'en'
                  ? <>Tapping “Start” begins a <strong>50-minute</strong> timer; after the in-app part, they write the long answers</>
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="bg-white rounded-2xl p-5 shadow-sm">
        <p className="text-sm text-gray-700 leading-6 mb-3">
          {lang === 'en' ? (
            <>Choose the math units covered by your child&apos;s upcoming exam. Once set, a{' '}
            <span className="font-semibold text-[#1D9E75]">&quot;Mock Exam Paper&quot;</span> appears on their home page.</>
          ) : (
        ...
        <div className="bg-[#F7FBF9] border border-[#1D9E75]/15 rounded-xl px-3 py-2.5">
          <p className="text-xs font-semibold text-[#1D9E75] mb-1.5">📝 {t('一份完整模擬考試包含')}</p>
          {lang === 'en' ? (
            <ul className="text-xs text-gray-600 space-y-1 leading-relaxed">
              ...
              <li>• <strong>50-minute</strong> limit; the timer pauses after the in-app part</li>
      ...
      <div className="bg-white rounded-2xl p-4 shadow-sm">
        <p className="text-xs font-semibold text-gray-400 mb-2 uppercase tracking-wide">{t('選擇子女')}</p>
          ...
          {linkedChildren.map((c) => (
            ...
            >
              ...
              <div>
                ...
                <p className="text-xs text-gray-400">{t(`小${GRADE_LABEL[c.grade ?? 0] ?? '—'}`)}</p>
              </div>
            </label>
          ))}
        </div>
      ...
      >
        {submitting
          ? t('儲存中…')
          : lang === 'en'
            ? `Confirm Exam Scope (${selectedUnitIds.size} units)`
            : `確認考試範圍（${selectedUnitIds.size} 個單元）`}
      </button>
    </form>
  )
}
