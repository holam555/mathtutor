...
3. solutions：${methodsCtx ? '優先使用上方「霖楓學苑專屬教學方法」中提供的方法名稱作為 title。如果無對應方法，再自擬四至六字方法名。' : '每個方法名稱必須是針對此題型的專業四至六字名稱（如「公分母先行法」「借位圖解訓練」「梯形分解法」）。'} detail 必須以「我們的[X系統]中有一套「[方法名稱]」」開頭，並包含一個具體操作步驟或計算示例。
