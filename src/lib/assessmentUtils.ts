...

{
  ...
  "weakAreas": [
    {
      "name": "範疇名稱（與輸入完全一致）",
      "priority": "急需加強|需要加強|可以加強（與輸入完全一致）",
      "errorTypes": [
...
如無強項或無弱項，對應陣列返回 []。禁止在JSON中使用任何markdown格式。`
  ...
  const response = await generateContentWithFallback(ai, {
    ...
    config: {
      ...
      temperature: 0.7,
      // Disable thinking — cuts latency from ~20s to ~5s on gemini-2.5-flash,
      // keeping the call within Vercel Hobby's 10s serverless limit.
